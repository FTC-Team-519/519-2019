...
public class Vuforia {
    ...
    public StoneSample stoneThread;
    ...
    public void sampleStonePosition(Telemetry telemetry) {
        try {
            VuforiaLocalizer.CloseableFrame frame = vuforia.getFrameQueue().take();
            stoneThread = new StoneSample(frame, telemetry);
            stoneThread.start();
        } catch (Exception e) {
            // guess
        }
...
class StoneSample extends Thread {
    ...
    public StoneSample(VuforiaLocalizer.CloseableFrame frame, Telemetry telemetry) {
        ...
        for (int i = 0; i < numImages; i++) {
            if (frame.getImage(i).getFormat() == PIXEL_FORMAT.RGB565) {
                ...
                this.telemetry.addData("Got The Image", "Success");
                break;
            }
        }
    ...
    private double getAmountOfYellow(Bitmap bm) {
        ...
        for (int y = 0; y < bm.getHeight(); y++) {
            for (int x = range; x < bm.getWidth(); x++) {
                ...
                this.telemetry.addData("Pixel (" + x + ", " + y + ")", "Y (" + yellow + ") ||| B (" + blue + ")");
                // telemetry.addData("Yellow Value", yellow + "");
                if (yellow > .7 && blue < 160) { //.8
                    amt += yellow;
                }
    ...
    public double yellowThirdOne;
    public double yellowThirdTwo;
    public double yellowThirdThree;
    ...
    public void run() {
        ...
        int boxWidth = bm.getWidth()/3;                      //    ||                   ||
        int boxHeight = bm.getHeight()/2;

        Bitmap thirdOneTop = Bitmap.createBitmap(bm, 0, boxHeight, boxWidth, boxHeight);
        Bitmap thirdTwoTop = Bitmap.createBitmap(bm, boxWidth, boxHeight, boxWidth, boxHeight);
        Bitmap thirdThreeTop = Bitmap.createBitmap(bm, boxWidth*2, boxHeight, boxWidth, boxHeight);

        yellowThirdOne = this.getAmountOfYellow(thirdOneTop);
        yellowThirdTwo = this.getAmountOfYellow(thirdTwoTop);
        yellowThirdThree = this.getAmountOfYellow(thirdThreeTop);

        this.telemetry.addData("Yellow Third One", yellowThirdOne + "");
        this.telemetry.addData("Yellow Third Two", yellowThirdTwo + "");
        this.telemetry.addData("Yellow Third Three", yellowThirdThree + "");
        position = "Left";
    }
}
